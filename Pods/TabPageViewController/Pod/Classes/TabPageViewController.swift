...
extension TabPageViewController {
    ...
    fileprivate func updateNavigationBar() {
        if let navigationBar = navigationController?.navigationBar {
            ...
            navigationBar.barTintColor = UIColor(red: 40/255, green: 156/255, blue: 182/255, alpha: 1.0)
            navigationBar.tintColor = UIColor.white
            UINavigationBar.appearance().titleTextAttributes = [NSFontAttributeName: UIFont(name: "AppleSDGothicNeo-semiBold", size: 20), NSForegroundColorAttributeName: UIColor.white]
            
            let backButton = UIBarButtonItem(image: UIImage(named: "25 25_b"), style: .plain, target: self, action: #selector(backAction))
            self.navigationItem.leftBarButtonItem = backButton
            
            navigationItem.title = "홍길동"
